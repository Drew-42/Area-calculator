...
import Area_calculator_v0_5 as area_calc

# This function will return a specific numerical value for each metric unit inputted. This method 
# is more generalized and could be used for other physical characteristics (volume, frequency, 
# power, energy, mass etc.), though in this app it is used only for units of distance
...
# Same as before but for imperial units of distance
def units_imperial_distance():
	units = {'twips': 0, 'points (Imperial)': 1, 'points (Digital Standard)': 2, 'points (European Traditional)': 3, 'lines/poppyseeds': 4, 'picas (Imperial)': 5, 'picas (Digital Standard)': 6, 'barleycorns': 7, 'digits': 8, 'fingers (cloth measure)': 9, 'fingers (fingerbreadth)': 10, 'inches': 11, 'sticks': 12, 'nails': 13, 'palms': 14, 'spans': 15, 'links': 16, 'shaftments': 17, 'feet': 18, 'hands': 19, 'cubits': 20, 'paces': 21, 'yards': 22, 'ells': 23, 'grades/steps': 24, 'fathoms': 25, 'ropes': 26, 'rods/poles/perches': 27, 'Ramsden\'s chains': 28, 'Gunter\'s chains': 29, 'shackles/shots': 30, 'furlongs': 31, 'cables (British Admiralty)': 32, 'cables (US Navy)': 33, 'Roman miles': 34, 'miles': 35, 'nautical miles': 36, 'leagues': 37, 'AU': 38, 'ly': 39, 'pc': 40}
	return units

# The numerical values returned by the previous functions serve as indexes in these unit 
# conversion tables (or ... tuples, rather). This first table contains how many of each unit 
# fit in a meter, from the smallest to the largest unit, left to right. It's used when 
# converting from meters to a metric unit or vice versa (with negative indexes)
...
# Same thing but for imperial units, first from imperial unit (smallest to largest - right 
# to left) to meteres and then vice versa (smallest imperial unit to largest - left to right) 
# (here negative indexes can't be used as before, because the relationships between imperial 
# units aren't as simple and ordered as those between metric units [powers of ten], which are 
# easily mirrorable)
...
# This function does the actual unit conversion. First it converts from the initial unit 
# (metric or imperial) to meters, and then from meters to the final unit of choice (metric or 
# imperial). I constructed it like this, as a 2-step process, to avoid building a single giant 
# conversion table (*look up*) with the relationships between all of the units, with too many 
# lines and columns (it is enough to have so many columns...). Instead, it was easier to add 
# a bit of code in this unit conversion function and make 3 small unit tables with 1 line each
...
# These functions calculate the area/volume of each shape, after it converts all inputed values 
# to the unit that was chosen to express the result (NOT the 'unit' seen here as parameter of 
# each function, rather it's the 'unit_final' parameter)
